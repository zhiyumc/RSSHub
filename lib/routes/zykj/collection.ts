...
import { MAIN_NEWS_CATEGORIES, DEPARTMENTS, NEWS_COLLECTION_KEYS, NOTICE_COLLECTION_KEYS } from './config';
import { parseList, type DepartmentConfig } from './utils';

// ===== News Collection Route =====

export const newsCollectionRoute: Route = {
...
- **媒体关注**：获取标题、作者、链接、发布日期、内容`,
    ...
    handler: newsCollectionHandler,
};

async function newsCollectionHandler(ctx) {
...
// ===== Notice Collection Route =====

export const noticeCollectionRoute: Route = {
...
所有条目获取：标题、作者、链接、发布日期、内容（链接内的内容）。`,
    ...
    handler: noticeCollectionHandler,
};

async function noticeCollectionHandler(ctx) {
